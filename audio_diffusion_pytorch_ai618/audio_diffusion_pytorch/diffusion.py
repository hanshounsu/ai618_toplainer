...
class ADPM2Sampler(Sampler):
    ...
    def step(self, x: Tensor, fn: Callable, sigma: float, sigma_next: float, sigma_last:float = None, return_x0=False) -> Tensor:
        # Sigma steps
        ...
        if return_x0:
            x_start = x + d_mid * (sigma_last - sigma)
            x_next = {"x_next": x_next, "pred_xstart": x_start}
            return x_next
        elif not return_x0:
            return x_next
        return x_next
    ...
    def inpaint(
        self,
        source: Tensor, # 2 channel audio with guitar
        mask: Tensor,
        fn: Callable,
        sigmas: Tensor,
        num_steps: int,
        ...
        for i in tqdm(range(num_steps - 1), desc='Inpainting in progress'):
            # Noise source to current noise level
            source_noisy = source + sigmas[i] * torch.randn_like(source)
            for r in range(num_resamples): # jump step 구현 필요.
    ...
    def inpaint_MCG(
        self,
        source: Tensor, # source = torch.cat((mel_prev, empty), dim=-1) # 1 x C x L
        mask: Tensor,
        fn: Callable, # denoise_fn -> predicts the denoised output
        sigmas: Tensor,
        num_steps: int,
        num_resamples: int,
        segment: int,  
    ) -> Tensor:
        mask = (mask==True).to(torch.int)
        x = sigmas[0] * torch.randn_like(source)
        for i in tqdm(range(num_steps - 1), desc='MCG inpainting in progress'):
            x = x.requires_grad_()
            # denoising step with denoising model fn
            out = self.step(x, fn=fn, sigma=sigmas[i], sigma_next=sigmas[i + 1], sigma_last=sigmas[-1], return_x0=True)  # type: ignore # noqa
            # l2norm(xa - xhat)
            q_sampled_x0 = source + sigmas[i] * torch.randn_like(x)
            # 이 태스크는 segment 상관없이 mcg 적용.
            # if segment > 0:
            norm = torch.norm((source * mask) - (out["pred_xstart"] * mask)) # ||y - P \hat{x_0}||^2_2
            # gradient w.r.t. z_t^b
            norm_grad = grad(outputs=norm, inputs=x)[0] * (1. - mask)
            out["x_next"] = out["x_next"] * (1. - mask) - norm_grad + \
                            q_sampled_x0 * mask
            # elif segment == 0:
            #     out["x_next"] = out["x_next"] * (1. - mask) + q_sampled_x0 * mask
...
class Diffusion(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        sigma_data: float,  # data distribution standard deviation -> hyperparameter
    ...
    def get_scale_weights(self, sigmas: Tensor) -> Tuple[Tensor, ...]:
        '''
        Check Table 1 at Karras et al.(2022)
        '''
    ...
    def denoise_fn(
        ...
        **kwargs,
        ...
        # Predict network output and add skip connection (Eq.(7) in Karras et al.(2022))
        c_skip, c_out, c_in, c_noise = self.get_scale_weights(sigmas)
        x_pred = self.net(c_in * x_noisy, c_noise, **kwargs) # predicting not exactly noise, but something inbetween noise and x_t 
        ...
        if self.dynamic_threshold == 0.0:
            ...
            x_flat = rearrange(x_denoised, "b ... -> b (...)") # B x C(=2) x L -> B x (C*L)
            scale = torch.quantile(x_flat.abs(), self.dynamic_threshold, dim=-1) # 작은 순으로, 0.95면 밑에서 0.95, 즉 상위 5%
            # Clamp to a min of 1.0 -> 상위 5% 값을 1보다는 큰 값으로 변환.
    ...
    def forward(self, x: Tensor, noise: Tensor = None, **kwargs) -> Tensor:
        ...
        sigmas = self.sigma_distribution(num_samples=batch, device=device) # time과 무관하게 양을 정하는 거 같은데..?
        sigmas_padded = rearrange(sigmas, "b -> b 1 1")
        ...
        losses = losses * self.loss_weight(sigmas) # Karras et al.(2022) Section 5
        loss = losses.mean()
...
class DiffusionInpainter(nn.Module):
    def __init__(
        ...
        *,
        ...
        elif sampling_method == 'MCG':
            self.inpaint_fn = sampler.inpaint_MCG
        else:
            raise ValueError("sampling method is not properly defined")
        print(colored('Inpainting method: ', 'red', attrs=['bold']) + sampling_method)
        # self.inpaint_fn = sampler.inpaint
        self.sigma_schedule = sigma_schedule

    # @torch.no_grad()
    def forward(self, inpaint: Tensor, inpaint_mask: Tensor, segment: int) -> Tensor:
        x = self.inpaint_fn(
            source=inpaint,
            mask=inpaint_mask,
            fn=self.denoise_fn,
