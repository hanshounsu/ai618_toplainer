...
#from utils.evaluation.audioldm_eval.audioldm_eval.feature_extractors.panns import Cnn14
...
class EvaluationHelper:
    def __init__(self, config, sampling_rate, device, backbone="cnn14") -> None:
        self.config = config
    ...
    def main(
        ...
        self.data_init_check(generated_files_path)
        self.data_init_check(groundtruth_path)

        same_name = self.get_filename_intersection_ratio(
            generated_files_path, groundtruth_path, limit_num=limit_num
        )

        metrics = self.calculate_metrics(generated_files_path, groundtruth_path, same_name, step, limit_num)

        return metrics

    def data_init_check(self, dir):
    ...
    def get_filename_intersection_ratio(
        ...
        data_dict1 = {'_'.join(os.path.basename(x).split('_')[:-2]): x for x in self.datalist1} # line for guitarset
        data_dict2 = {os.path.splitext(os.path.basename(x))[0]: x for x in self.datalist2} # line for guitarset
    ...
    def calculate_metrics(self, generate_files_path, groundtruth_path, same_name, step, limit_num=None):
        # Generation, target
        torch.manual_seed(0)
        ...
        json_path = generate_files_path + '_json_' + f"/{step}_same_name_{same_name}.json"
        os.makedirs(os.path.dirname(json_path), exist_ok=True)
        write_json(result, json_path)

        if same_name == False and self.config.wandb == True:
            wandb.log({'frechet_distance': out.get("frechet_distance", float("nan")),
                        'frechet_audio_distance': out.get("frechet_audio_distance", float("nan")),
                        'kullback_leibler_divergence_sigmoid': out.get("kullback_leibler_divergence_sigmoid", float("nan")),
                        'kullback_leibler_divergence_softmax': out.get("kullback_leibler_divergence_softmax", float("nan")),
                        "kernel_inception_distance_mean": out.get(
                            "kernel_inception_distance_mean", float("nan")
                        ),
                        "kernel_inception_distance_std": out.get(
        ...
        if same_name == True and self.config.wandb == True:
            wandb.log({'frechet_distance_eval': out.get("frechet_distance", float("nan")),
                'frechet_audio_distance_eval': out.get("frechet_audio_distance", float("nan")),
                'kullback_leibler_divergence_sigmoid_eval': out.get("kullback_leibler_divergence_sigmoid", float("nan")),
                'kullback_leibler_divergence_softmax_eval': out.get("kullback_leibler_divergence_softmax", float("nan")),
                'inception_score_mean_eval': out.get("inception_score_mean", float("nan")),
                'inception_score_std_eval': out.get("inception_score_std", float("nan")),
                "kernel_inception_distance_mean_eval": out.get(
                    "kernel_inception_distance_mean_eval", float("nan")
                ),
                "kernel_inception_distance_std_eval": out.get(
                    "kernel_inception_distance_std_eval", float("nan")
                ),
                "evaluation_step" : step
                })
        return result
    ...
    def get_featuresdict(self, dataloader):
        ...
        for waveform, filename in tqdm(dataloader):
            try:
                ...
                featuresdict = {k: [v.cpu()] for k, v in featuresdict.items()} # 왜 cpu?? -> FID, FD가 numpy로 계산됨.
