...
from utils.evaluation.audioldm_eval.audioldm_eval.feature_extractors.panns.pytorch_utils import (
    do_mixup,
    interpolate,
...
class Cnn14(nn.Module):
    def __init__(
        ...
        if not os.path.exists("utils/evaluation/audioldm_eval/ckpt/Cnn14_mAP=0.431.pth"):
            print("Download pretrained checkpoints of Cnn14.")
            os.makedirs("ckpt", exist_ok=True)
            os.system(
                "wget -P utils/evaluation/audioldm_eval/ckpt/ %s"
        ...
        if sample_rate == 16000:
            state_dict = torch.load("./utils/evaluation/audioldm_eval/ckpt/Cnn14_16k_mAP=0.438.pth", map_location="cuda")
            self.load_state_dict(state_dict["model"])
        elif sample_rate == 32000:
            state_dict = torch.load("./utils/evaluation/audioldm_eval/ckpt/Cnn14_mAP=0.431.pth", map_location="cuda")
            self.load_state_dict(state_dict["model"])
...
class Cnn14_16k(nn.Module):
    def __init__(
        ...
        if not os.path.exist("utils/evaluation/audioldm_eval/ckpt/Cnn14_mAP=0.431.pth"):
            print("Download pretrained checkpoints of Cnn14.")
            os.makedirs("ckpt", exist_ok=True)
            os.system(
                "wget -P utils/evaluation/audioldm_eval/ckpt/ %s"
        ...
        # self.init_weight()
        if sample_rate == 16000:
            state_dict = torch.load("utils/evaluation/audioldm_eval/ckpt/Cnn14_16k_mAP=0.438.pth")
            self.load_state_dict(state_dict["model"])
        elif sample_rate == 32000:
            state_dict = torch.load("utils/evaluation/audioldm_eval/ckpt/Cnn14_mAP=0.431.pth")
            self.load_state_dict(state_dict["model"])
