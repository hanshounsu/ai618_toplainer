...
class JupyterNotebookUtil():
    ...
    @staticmethod
    def get_html_from_src_path(type:str,path:str,width:int=150) -> str:
        path = quote(path)
        if type == "audio":
            if width is not None:
                return f"""<audio controls style='width:{width}px'><source src="{path}" type="audio/wav"></audio>"""
            else:
                return f"""<audio controls><source src="{path}" type="audio/wav"></audio></td>"""
        elif type == "img":
            if width is not None:
                # return f"""<img src="{path}">"""
                return f"""<img src="{path}" style='width:{width}px !important;'>"""
