...
class Predictor(MusicDemixingPredictor):
    ...
    def prediction_setup(self):
        self.models = get_models(model_name, load=False, device=device_)
    ...
    def prediction(self, mixture_file_path, bass_file_path, drums_file_path, other_file_path, vocals_file_path):
        ...
        if mix.shape[0] > 1.9e+7: # 6.5분 이상
            print(colored(f'Skipping {mixture_file_path}; Too long, memory will blow!!', 'red'))
            pass
        else:
    ...
    def demix_base(self, mix):
        ...
        for model in self.models:
            ...
            while i < n_sample + pad:
                ...
                i += gen_size            
            mix_waves = torch.tensor(np.array(mix_waves), dtype=torch.float32, device=device_)    

            with torch.no_grad():
                _ort = ort.InferenceSession(f'onnx/{model.target_name}.onnx')
                tar_waves = model.istft(torch.tensor(
                    _ort.run(None, {'input': model.stft(mix_waves).cpu().numpy()})[0], device=device_))
